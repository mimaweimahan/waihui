...
if (!function_exists('setProduct')) {
     ...
     */
    function setProduct($refresh = false)
    {
        //获取服务器编号
        $server_number = \think\Env::get('server.number', '');
        ...
        //判断台子是否存入产品数据
        if ($redis->has($server_number . '_product') && !$refresh) {
            return true;
        }

        //获取产品列表
        $list = \app\admin\model\Product::where(['status' => 1])->field('type,code')->select();

        $product = [];
        foreach ($list as $item) {
            $product [$item ['type']][$item ['code']] = $item ['code'];
        }

        foreach ($product as $key => $value) {
            $redis->set($server_number . '_product_' . $key, json_encode($value, JSON_UNESCAPED_UNICODE));
        }

        $redis->set($server_number . '_product', $server_number . '台子已存入产品数据');
        return true;
    }
}
